...
from anode.viz.plots import get_square_aspect_ratio


def feature_evolution_gif(
    feature_history, targets, dpi=100, alpha=0.5, filename="feature_evolution.gif"
):
    ...
    if not filename.endswith(".gif"):
        raise RuntimeError(
            "Filename must end in with .gif, but filename is {}".format(filename)
        )
